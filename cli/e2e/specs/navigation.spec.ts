...
test.describe('Navigation', () => {
  ...
  test('should navigate to patients page', async ({ authenticatedPage }) => {
    ...
    const hasPatientsContent = await authenticatedPage.locator('text=/patient/i, [data-testid="patients"], h1, h2').first().isVisible().catch(() => false);
  ...
  test('should redirect to access denied for unauthorized routes', async ({ page, loginAs }) => {
    ...
    try {
      ...
      const isAccessDenied = page.url().includes('access-denied') || 
                            await page.locator('text=/access denied|unauthorized|permission/i').first().isVisible().catch(() => false);
  ...
  test('should show sidebar navigation', async ({ authenticatedPage }) => {
    ...
    const sidebar = authenticatedPage.locator('[data-testid="sidebar"], nav, aside, [role="navigation"]').first();
