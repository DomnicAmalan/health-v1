...
test.describe('Mobile UI @mobile', () => {
  test('should display mobile-friendly navigation', async ({ authenticatedPage }) => {
    ...
    const mobileMenu = authenticatedPage.locator('[data-testid="mobile-menu"], button[aria-label*="menu"], [aria-label*="Menu"]').first();
  ...
  test('should handle mobile keyboard input', async ({ authenticatedPage }) => {
    ...
    // Find an input field
    const input = authenticatedPage.locator('input[type="text"], input[type="email"], input[type="search"]').first();
