...
import { I18nContext, getTranslation, formatDateInLocale, formatNumberInLocale, formatCurrencyInLocale, isRTL, getLocaleInfo } from './i18n';
...
export function TranslationProvider({ children, defaultLocale = 'en' }: TranslationProviderProps) {
  ...
  const value = useMemo(
    () => ({
      ...
      formatDate: (date: Date, options?: Intl.DateTimeFormatOptions) => formatDateInLocale(locale, date, options),
      formatNumber: (value: number, options?: Intl.NumberFormatOptions) => formatNumberInLocale(locale, value, options),
      formatCurrency: (value: number, currency?: string) => formatCurrencyInLocale(locale, value, currency),
      isRTL: () => isRTL(locale),
