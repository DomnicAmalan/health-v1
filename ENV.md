...
- **No docker-compose defaults**: All required variables MUST be in `.env`

## Quick Start

### 1. Set up root environment file

```bash
# Copy example file (if you're a new developer)
cp .env.example .env
...
**IMPORTANT:** Change this password immediately in production! This is for development only.
...
- **`<SERVICE>_PORT`**: External/exposed port (what you access from host)
- **`SERVER_PORT`**: Internal port backend listens on (inside Docker)
...
These variables have NO defaults in docker-compose. Docker will fail if they're missing.
...
All frontend variables use `VITE_` prefix for build-time injection.

#### Admin UI (.env location: `cli/packages/apps/admin/.env`)
```bash
VITE_PORT=5174
...
#### Client App (.env location: `cli/packages/apps/client-app/.env`)
```bash
VITE_PORT=5175
VITE_API_BASE_URL=http://localhost:8080          # → API Service (8080)
VITE_OIDC_ISSUER=http://localhost:8080
...
#### RustyVault UI (.env location: `cli/packages/apps/rustyvault-ui/.env`)
```bash
VITE_PORT=8215
VITE_API_BASE_URL=http://localhost:4117/v1       # → Vault Service (4117, NOT 8080!)
# Note: Vault UI doesn't use OIDC
```
...
**Admin & Client apps** connect to the **API Service** (port 8080):
```bash
...
**Missing a URL?** You'll get CORS errors in the browser console.
...
**Solution:** Add the missing variable to root `.env` file. This is **intentional** - required variables must be explicitly set.
...
**Solution:** Check `CORS_ALLOWED_ORIGINS` in root `.env` includes the frontend URL:
```bash
...
**Solution:** Verify `VITE_API_BASE_URL` in app `.env` file:

- **Admin & Client apps** should use `http://localhost:8080` (API Service)
...
Or change the port in the app's `.env` file (and update CORS + redirect URIs accordingly).

### Authentication redirect fails

**Symptom:** After login, redirect goes to wrong URL or fails
...
**Symptom:** Vault UI can't connect to backend

**Solution:** Verify Vault UI is using the correct API URL:
```bash
# In cli/packages/apps/rustyvault-ui/.env
VITE_API_BASE_URL=http://localhost:4117/v1    # Must include /v1 prefix!
```

Also ensure RustyVault service is running:
```bash
...
**Cause:** Vite is trying to auto-open a browser inside Docker container (which has no display)

**Solution:** Set `VITE_OPEN=false` in app `.env` file:
```bash
# In cli/packages/apps/*/. env
VITE_OPEN=false    # Don't auto-open browser
```

**Note:** When running Vite dev servers locally (outside Docker) with `make dev-*`, you can set `VITE_OPEN=true` to auto-open your browser.
...
- [ ] Admin & Client use `VITE_API_BASE_URL=http://localhost:8080`
...
If you need to change ports (e.g., 5174 is already in use):
...
- `COMMANDS.md` - Comprehensive Makefile command reference
...
- Review error messages carefully (they indicate missing variables)
