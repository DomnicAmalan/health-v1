...
impl TokenManager {
    ...
    pub fn generate_access_token_with_permissions(
        ...
        encode(&Header::default(), &claims, &self.encoding_key)
            .map_err(|e| shared::AppError::Authentication(format!("Token generation failed: {}", e)))
    }
    ...
    pub fn generate_refresh_token(&self, user: &User) -> AppResult<String> {
        ...
        encode(&Header::default(), &claims, &self.encoding_key)
            .map_err(|e| shared::AppError::Authentication(format!("Refresh token generation failed: {}", e)))
    }
